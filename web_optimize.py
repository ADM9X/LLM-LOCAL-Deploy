...
check_args(model_name)
model_data = data_dir / model_name

tokenizer = AutoTokenizer.from_pretrained(model_data, trust_remote_code=True)
model = AutoModelForCausalLM.from_pretrained(model_data, trust_remote_code=True).cuda()
...
def predict(input, chatbot, max_length=100, top_p=0.7, temperature=0.0, history=[], past_key_values=None):
...
with gr.Blocks() as demo:
    gr.HTML("""<h1 align="center">ChatGLM3_6B</h1>""")
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            max_length = gr.Slider(0, 32768, value=8192, step=1.0, label="Maximum length", interactive=False)
